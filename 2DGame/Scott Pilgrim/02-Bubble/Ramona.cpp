...
void Ramona::update(int deltaTime)
{
	...
	if (vida <= 0) {
		...
		if (movimiento == 13 || movimiento == 14 && (sprite_recibir->animation() == 2 || sprite_recibir_izq->animation() == 2)) {
			...
			if (sprite_recibir->animation() == 2 && movimiento == 14 && recuperando < 1500 && level != 3 && posPlayer.x > 0) posPlayer.x -= 1.5f;
...
void Ramona::recibirPuñetazoIzquierda(int vida_quitada) {
...
void Ramona::recibirPuñetazoDerecha(int vida_quitada) {
